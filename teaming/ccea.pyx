...
def assignBestCceaPolicies2(data):
    number_agents = data['Number of Agents']
    populationCol = data['Agent Populations']
    policyCol = [None] * number_agents
    for agentIndex in range(number_agents):
        policyCol[agentIndex] = max(populationCol[agentIndex], key = lambda policy: len(policy.life))
        #policyCol[agentIndex] = populationCol[agentIndex][0]
    data["Agent Policies"] = policyCol

def assignCceaPoliciesHOF2(data):
    assignBestCceaPolicies2(data)
    curAgent = data["Current Agent"]
    worldIndex = data["World Index"]
    populationCol = data['Agent Populations']
...
def rewardCceaPoliciesHOF(data,G):
    policyCol = data["Agent Policies"]
    curAgent = data["Current Agent"]
    number_agents = data['Number of Agents']
    rewardCol = data["Agent Rewards"]
    
    policyCol[curAgent].life.append(G)
    policyCol[curAgent].fitness = np.mean(policyCol[curAgent].life)

def rewardCceaPoliciesHOF2(data,G,quantile):
    policyCol = data["Agent Policies"]
    curAgent = data["Current Agent"]
    number_agents = data['Number of Agents']
    rewardCol = data["Agent Rewards"]
    ...
    policyCol[curAgent].life.append(G)
    policyCol[curAgent].fitness = np.quantile(policyCol[curAgent].life,quantile)

def rewardCceaPoliciesHOF3(data,G):
    policyCol = data["Agent Policies"]
    curAgent = data["Current Agent"]
    number_agents = data['Number of Agents']
    rewardCol = data["Agent Rewards"]
    ...
    policyCol[curAgent].life.append(G)
    policyCol[curAgent].fitness = np.median(policyCol[curAgent].life)
